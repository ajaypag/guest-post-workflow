...
// This page shows the comparison between current guest_post_cost and derived prices
...
export default function PricingComparisonTestPage() {
  ...
  const fetchComparisons = async () => {
    try {
      ...
      console.error('Failed to fetch pricing comparisons:', err);
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`px-2 py-1 rounded text-xs font-medium ${badges[status as keyof typeof badges] || 'bg-gray-100 text-gray-800'}`}>
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-4">Phase 6A: Guest Post Cost vs Derived Price Comparison</h1>
            ...
            {filteredComparisons.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-6 py-4 text-center text-gray-500">
                  No websites found for the selected filter
                </td>
              </tr>
            ) : (
              filteredComparisons.map((comp) => (
                <tr key={comp.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {comp.offeringPrices && comp.offeringPrices.length > 1 && (
                  ...
                  <td className="px-6 py-4">
                    <div className="text-xs text-gray-600">
                      {comp.publisherNames && comp.publisherNames.length > 0 
      ...
      {filter !== 'match' && (stats.missingOfferings > 0 || stats.mismatches > 0) && (
