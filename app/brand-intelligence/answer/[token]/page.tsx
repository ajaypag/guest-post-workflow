...
import { CheckCircle, AlertCircle, Loader2, Send, ArrowRight, Download } from 'lucide-react';
...
export default function AnswerQuestionsPage() {
  ...
  const loadQuestions = async () => {
    try {
      const response = await fetch(`/api/brand-intelligence/questions/${token}`);
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `brand-questions-${questionData.client.name.replace(/\s+/g, '-')}.csv`);
  ...
  const submitAnswers = async () => {
    try {
      ...
      const response = await fetch(`/api/brand-intelligence/submit-answers/${token}`, {
        method: 'POST',
        ...
        <div className="flex items-center justify-center py-20">
          <div className="max-w-md mx-auto text-center p-8">
            ...
            <p className="text-gray-600 mb-6">
              Your answers have been submitted successfully. Our team will use this information to create a comprehensive brand brief for <strong>{questionData?.client.name}</strong>.
            </p>
            <p className="text-sm text-gray-500">
              You'll be notified when the brand brief is ready for review.
            </p>
          </div>
        </div>
        <MarketingFooter />
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white">
      ...
      <div className="max-w-4xl mx-auto py-12 px-4">
        ...
        <div className="text-center mb-12">
          <h1 className="text-3xl font-bold text-gray-900 mb-4">Brand Intelligence Questions</h1>
          <p className="text-lg text-gray-600 mb-2">
            Help us create the perfect brand brief for <span className="font-semibold text-blue-600">{questionData?.client.name}</span>
          </p>
          <p className="text-sm text-gray-500">
            These questions help us understand your business better and ensure accurate representation in all content.
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <p className="text-sm text-gray-600 mb-4">
                Our AI research findings about your business. Review and edit this analysis to ensure accuracy (questions are handled separately below):
              </p>
            </div>
            <button
          ...
          <div className="flex justify-between text-xs text-gray-500 mt-2">
            <span>Edit any inaccuracies or add missing information • Use the resize handle or expand button for better editing</span>
            <span>{editedResearch.length} characters</span>
          </div>
        </div>

        {/* Additional Business Information - Free-form */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
          <div className="mb-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Additional Business Information</h3>
            <p className="text-sm text-gray-600 mb-4">
              What else should we know about your business? Add any important details you want mentioned in articles:
            </p>
          </div>
          <textarea
            value={additionalInfo}
            onChange={(e) => setAdditionalInfo(e.target.value)}
            placeholder="e.g., Key achievements, awards, unique selling points, company culture, recent news, partnerships, client testimonials, etc."
          ...
          />
          <div className="flex justify-between text-xs text-gray-500 mt-2">
            <span>This information will help create more comprehensive and accurate articles</span>
