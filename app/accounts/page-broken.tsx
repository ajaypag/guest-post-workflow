...
function AccountsPageContent() {
  ...
  const startImpersonation = async (account: Account) => {
    if (!confirm(`Start impersonating ${account.contactName} (${account.email})?`)) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        showNotification('success', `Now impersonating ${account.contactName}`);
        ...
        const error = await response.json();
        showNotification('error', error.error || 'Failed to start impersonation');
      }
    } catch (error) {
      console.error('Error starting impersonation:', error);
      showNotification('error', 'Failed to start impersonation');
    } finally {
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center space-x-3">
                        ...
                        <button
                          ...
                          disabled={impersonating === account.id}
                          className="text-green-600 hover:text-green-900 disabled:opacity-50"
                          title="Impersonate User"
                        >
                          {impersonating === account.id ? (
