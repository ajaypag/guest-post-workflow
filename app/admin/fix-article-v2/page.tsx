...
import { Zap, AlertTriangle, CheckCircle, Database, Wrench, RefreshCw } from 'lucide-react';
...
export default function FixArticleV2Page() {
  const [isLoading, setIsLoading] = useState(false);
  const [diagnostics, setDiagnostics] = useState<DiagnosticResult | null>(null);
  const [columnFixResult, setColumnFixResult] = useState<any>(null);
  ...
  const runDiagnostics = async () => {
    ...
    try {
      const response = await fetch('/api/admin/diagnose-article-v2');
  ...
  const fixColumns = async () => {
    ...
    try {
      const response = await fetch('/api/admin/fix-article-v2-columns', {
        method: 'POST'
      });
      const data = await response.json();
      
      if (response.ok && data.success) {
        setColumnFixResult(data);
        // Re-run diagnostics to see updated state
        await runDiagnostics();
      } else {
        setError(data.error || 'Failed to fix columns');
      }
    } catch (err) {
      setError(`Error: ${err instanceof Error ? err.message : 'Unknown error'}`);
    } finally {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'orchestrating': return 'text-blue-600';
          ...
          <div className="border-b border-gray-200 px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <h1 className="text-xl font-semibold text-gray-900">Article V2 Diagnostics</h1>
              </div>
              <button
          ...
          {columnFixResult && (
            <div className="mx-6 mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center">
                ...
                <p className="text-green-800">{columnFixResult.message}</p>
              </div>
            </div>
          )}
          ...
          {diagnostics && (
            <div className="p-6 space-y-6">
              ...
              <div className="border border-gray-200 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{diagnostics.tableStatus.columns.length}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Indexes:</p>
                    <p className="font-medium">{diagnostics.tableStatus.indexes.length}</p>
                  </div>
                </div>
              </div>
              ...
              {diagnostics.columnAnalysis.issues.length > 0 && (
                <div className="border border-amber-200 bg-amber-50 rounded-lg p-4">
                  ...
                  <button
                    onClick={fixColumns}
                    disabled={isLoading}
                    className="mt-4 inline-flex items-center px-4 py-2 bg-amber-600 text-white text-sm font-medium rounded-md hover:bg-amber-700 disabled:opacity-50"
                  >
                    <Wrench className="w-4 h-4 mr-2" />
                    {isLoading ? 'Fixing...' : 'Fix Column Sizes'}
                  </button>
                </div>
              )}
              ...
              {diagnostics.sampleData.latestSessions.length > 0 && (
                <div className="border border-gray-200 rounded-lg p-4">
                  <h3 className="font-semibold text-gray-900 mb-3">Latest V2 Sessions</h3>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          ...
                          <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Words</th>
                        ...
                        {diagnostics.sampleData.latestSessions.map((session: any, idx: number) => (
                          <tr key={idx}>
                            ...
                            <td className="px-3 py-2 text-sm text-gray-900">
                              {session.completed_sections || 0}/{session.total_sections || '?'}
                            </td>
                            <td className="px-3 py-2 text-sm text-gray-900">
                              {session.current_word_count || 0}/{session.total_word_count || '?'}
                            </td>
