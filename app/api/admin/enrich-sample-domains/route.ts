  ...
  try {
    ...
    for (const domain of domainsToEnrich) {
      ...
      enrichedDomains.push({
        ...
        status: enrichmentData.qualificationStatus
      });
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully enriched ${enrichedCount} domains with AI qualification data`,
      enrichedDomains
    });
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to enrich domains' },
      { status: 500 }
    );
  }
}