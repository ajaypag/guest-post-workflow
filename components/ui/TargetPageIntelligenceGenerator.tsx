...
interface TargetPageIntelligenceGeneratorProps {
  targetPageId: string;  // Changed from clientId
  targetUrl: string;     // Added for display
...
export function TargetPageIntelligenceGenerator({ targetPageId, targetUrl, clientId, onComplete, userType = 'internal' }: TargetPageIntelligenceGeneratorProps) {
  // Status tracking
  ...
  useEffect(() => {
    const loadExistingSession = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`/api/target-pages/${targetPageId}/intelligence/latest`);
        ...
        if (data.success && data.session) {
          ...
          console.log('Loading existing target page intelligence session:', session.id, {
  ...
  const startResearch = async () => {
    try {
      ...
      setError('');
      setProgress('Starting deep research analysis of this target page...');
      setCurrentPhase('research');

      const response = await fetch(`/api/target-pages/${targetPageId}/intelligence/start-research`, {
        method: 'POST',
      });

      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Failed to start target page intelligence research');
      }
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (data.briefStatus === 'completed') {
          ...
          setCurrentPhase('completed');
          setProgress('Target page intelligence completed successfully!');
          ...
          if (!data.finalBrief) {
            ...
            setTimeout(async () => {
              try {
                const reloadResponse = await fetch(`/api/target-pages/${targetPageId}/intelligence/latest`);
  ...
  const submitClientInput = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/target-pages/${targetPageId}/intelligence/submit-input`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
  ...
  const generateBrief = async () => {
    try {
      ...
      setError('');
      setProgress('Generating comprehensive target page brief...');

      const response = await fetch(`/api/target-pages/${targetPageId}/intelligence/generate-brief`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sessionId })
      });
  ...
  const saveBriefEdit = async () => {
    try {
      const response = await fetch(`/api/target-pages/${targetPageId}/intelligence/brief`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
    ...
    <div className="space-y-6">
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
        <div className="flex items-center space-x-2 mb-2">
          <span className="text-sm font-medium text-purple-900">Target Page:</span>
          <a 
            href={targetUrl} 
            target="_blank" 
        ...
        <p className="text-xs text-purple-700">
          This intelligence focuses specifically on this target page rather than the entire brand.
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <p className="text-sm text-gray-600">
          <strong className="text-gray-700">Why this matters:</strong> This comprehensive target page brief helps us understand this specific product/service deeply 
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <ul className="text-sm text-amber-700 space-y-1 list-disc list-inside">
                      <li>Deep analysis of this specific target page and related pages</li>
                      <li>Research third-party mentions and reviews about this product/service</li>
                      <li>Analyze competitor offerings in this space</li>
              ...
              {userType === 'internal' ? (
                ...
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <p className="text-sm text-gray-600">
                    Target page intelligence research needs to be initiated by your account manager. 
      ...
      {currentPhase !== 'input' && !researchOutput && currentPhase !== 'brief' && currentPhase !== 'completed' && (
        <div className="bg-white border border-gray-200 rounded-lg p-6 opacity-50">
          ...
          <p className="text-sm text-gray-400">
            After research completes, you'll answer specific questions about this target page to fill any information gaps.
          </p>
        </div>
      )}
      ...
      {currentPhase !== 'brief' && currentPhase !== 'completed' && !finalBrief && (
        <div className="bg-white border border-gray-200 rounded-lg p-6 opacity-50">
          ...
          <p className="text-sm text-gray-400">
            We'll combine research findings with your input to create a comprehensive target page brief that guides all content creation.
          </p>
        </div>
      )}

      {/* Phase 2: Questionnaire (Active) - This section continues with similar adaptations... */}
      {/* For brevity, I'll continue with the key sections that need changes */}
                  ...
                  {researchOutput.gaps && researchOutput.gaps.length > 0 && userType === 'internal' && (
                    <div className="flex items-center space-x-2">
                      ...
                      >
                        ...
                        {isSendingEmail ? 'Sending...' : emailSent ? 'Email Sent!' : sessionData?.metadata?.answerToken ? 'Resend to Client' : 'Send to Client'}
                      </button>
                    </div>
                  )}
                  ...
                  {showExistingLink && sessionData?.metadata?.answerToken && (
                    <div className="mt-3 p-3 bg-purple-50 border border-purple-200 rounded-lg relative">
                      ...
                      <div className="flex items-center space-x-2">
                        <input
                          type="text"
                          value={`${process.env.NEXT_PUBLIC_APP_URL || window.location.origin}/target-page-intelligence/answer/${sessionData.metadata.answerToken}`}
                          readOnly
                          className="flex-1 text-xs font-mono bg-white border border-purple-300 rounded px-2 py-1 text-purple-800"
                        />
                        <button
                          onClick={() => {
                            const url = `${process.env.NEXT_PUBLIC_APP_URL || window.location.origin}/target-page-intelligence/answer/${sessionData.metadata.answerToken}`;
                ...
                {clientInput ? (
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <p className="text-sm text-blue-800">
                      <strong>Your Input Needed:</strong> Please answer each question below to help us create an accurate brief for this target page.
                    </p>
                  </div>
                )}
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Your input has been submitted. Generate the comprehensive target page brief.
                  </p>
                </div>
                <button
      ...
      {currentPhase === 'input' && !briefStatus && (
        <div className="bg-white border border-gray-200 rounded-lg p-6 opacity-50">
          ...
          <p className="text-sm text-gray-400">
            After you submit your input, we'll create a comprehensive target page brief combining all research and your answers.
          </p>
        </div>
      )}
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          {['queued', 'in_progress'].includes(briefStatus) && (
            <div className="space-y-4">
              <div className="flex items-center space-x-3">
                <Loader2 className="w-5 h-5 text-blue-500 animate-spin" />
                <span className="font-medium">Generating Target Page Brief</span>
              </div>
              <p className="text-sm text-gray-600">{progress}</p>
              <div className="text-xs text-gray-500">
                Status checks: #{pollingCount} • Elapsed: {formatTime(pollingCount * 5)}
        ...
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-lg">
            <div className="border-b border-gray-200 px-4 py-3 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <CheckCircle className="w-5 h-5 text-green-500" />
                <h4 className="font-medium text-gray-900">Target Page Intelligence Complete</h4>
              </div>
              <div className="flex items-center space-x-2">
                <button
                  onClick={() => {
