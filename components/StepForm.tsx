...
const stepForms: Record<string, React.FC<{ step: WorkflowStep; workflow: GuestPostWorkflow; onChange: (data: any) => void }>> = {
  ...
  'keyword-research': ({ step, workflow, onChange }) => {
    ...
    const safeOutputs = {
      keywords: step.outputs?.keywords || '',
      csvExported: step.outputs?.csvExported || '',
      clientUrls: step.outputs?.clientUrls || '',
      urlSummaries: step.outputs?.urlSummaries || '',
      ...step.outputs
    };
    
    console.log('keyword-research initialized with:', safeOutputs);
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <textarea
          value={safeOutputs.keywords}
          onChange={(e) => onChange({ ...safeOutputs, keywords: e.target.value })}
          className="w-full px-3 py-2 border rounded-md h-32"
          placeholder="Paste the list of keywords generated by ChatGPT"
        />
        {safeOutputs.keywords && (
          <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded">
            <p className="text-xs font-semibold text-green-800 mb-1">✅ Saved Data:</p>
            <div className="text-xs text-green-700 max-h-20 overflow-y-auto whitespace-pre-wrap">
              {safeOutputs.keywords}
            </div>
          </div>
        )}
      ...
      <div>
        ...
        <select
          value={safeOutputs.csvExported}
          onChange={(e) => onChange({ ...safeOutputs, csvExported: e.target.value })}
          className="w-full px-3 py-2 border rounded-md"
        >
      ...
      <div>
        ...
        <textarea
          value={safeOutputs.clientUrls}
          onChange={(e) => onChange({ ...safeOutputs, clientUrls: e.target.value })}
      ...
      <div>
        ...
        <textarea
          value={safeOutputs.urlSummaries}
          onChange={(e) => onChange({ ...safeOutputs, urlSummaries: e.target.value })}
          className="w-full px-3 py-2 border rounded-md h-32"
          placeholder="Paste the output with target URLs and summaries"
        />
        {safeOutputs.urlSummaries && (
          <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded">
            <p className="text-xs font-semibold text-green-800 mb-1">✅ Saved Data:</p>
            <div className="text-xs text-green-700 max-h-20 overflow-y-auto whitespace-pre-wrap">
              {safeOutputs.urlSummaries}
