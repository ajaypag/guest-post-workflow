...
export const outlineSessions = pgTable('outline_sessions', {
  ...
  lastSequenceNumber: integer('last_sequence_number').default(0), // Track streaming sequence for resume
  connectionStatus: varchar('connection_status', { length: 50 }), // 'preparing' | 'connected' | 'disconnected' | 'completed' | 'failed'
  streamStartedAt: timestamp('stream_started_at'), // When streaming began
  partialContent: text('partial_content'), // Accumulated streaming content
  errorMessage: text('error_message'),
