...
Begin your research now.`;
  ...
  async resumeStream(sessionId: string, fromSequenceNumber: number): Promise<{
    status: string;
    content?: string;
    sequenceNumber?: number;
  }> {
    try {
      ...
      // If session is completed, return final content
      if (session.status === 'completed') {
        return {
          status: 'completed',
          content: session.finalOutline || '',
          sequenceNumber: session.lastSequenceNumber || 0
        };
      }

      // If session is still streaming, return current partial content
      if (session.status === 'streaming_started' || session.status === 'in_progress') {
        return {
          status: session.status,
