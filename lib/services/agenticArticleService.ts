...
CRITICAL: When you extract sections from the outline, you MUST also extract the detailed content requirements for each section. For example, if the outline mentions what to put in a section, you must include that information in the content_requirements field for that section. Do not just extract titles and word counts - extract the complete details for that section so the tool can better produce content. Also, you are encouraged to review the entire outline when extracting details for a section so that you can add some additional detail, narrative, or insights that would be useful for this specific section. If the outline contains numbered lists, tips, rankings, or "best of" content, treat each individual item as its own section for detailed coverage. Don't group multiple list items into a single section - each item deserves focused attention.
