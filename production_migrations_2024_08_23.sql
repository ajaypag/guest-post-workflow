-- =========================================================================
-- Production Migrations - August 23, 2024
-- =========================================================================
-- This file contains all migrations needed for production deployment
-- From branch: bug-fixing
-- Migrations: 0060 through 0070
-- =========================================================================
...
-- =========================================================================
-- Add columns for AI-powered target URL matching to bulk_analysis_domains
...
-- =========================================================================
-- Update existing NULL inclusion_status values to 'included' for better UX
...
-- Insert migration record
INSERT INTO migrations (name, applied_at) 
VALUES ('0061_fix_inclusion_status_defaults', NOW())
ON CONFLICT (name) DO NOTHING;
...
-- =========================================================================
-- Migration 0063: Add Workflow Assignment Tracking
-- =========================================================================
...
-- =========================================================================
-- Add fields for publisher coordination on orders
ALTER TABLE orders
ADD COLUMN IF NOT EXISTS publisher_notes TEXT,
ADD COLUMN IF NOT EXISTS publisher_contacted_at TIMESTAMP,
ADD COLUMN IF NOT EXISTS publisher_responded_at TIMESTAMP,
ADD COLUMN IF NOT EXISTS publisher_coordinator_id UUID REFERENCES users(id);

-- Add fields for line items
ALTER TABLE order_line_items
ADD COLUMN IF NOT EXISTS publisher_contacted_at TIMESTAMP,
ADD COLUMN IF NOT EXISTS publisher_responded_at TIMESTAMP,
ADD COLUMN IF NOT EXISTS publisher_notes TEXT;
...
-- Insert migration record
INSERT INTO migrations (name, applied_at) 
VALUES ('0064_add_publisher_coordination_fields', NOW())
ON CONFLICT (name) DO NOTHING;
...
-- Add delivered_at and client_notified_at if they don't exist
ALTER TABLE orders
ADD COLUMN IF NOT EXISTS delivered_at TIMESTAMP,
ADD COLUMN IF NOT EXISTS client_notified_at TIMESTAMP;
...
-- =========================================================================
-- Migration 0066: Populate Workflow Assignments (Data Migration)
-- =========================================================================
-- Populate workflow_assignments from existing assigned_user_id
INSERT INTO workflow_assignments (workflow_id, user_id, assigned_at, status)
SELECT 
    id as workflow_id,
    assigned_user_id as user_id,
    COALESCE(updated_at, created_at) as assigned_at,
    'active' as status
FROM workflows
...
AND NOT EXISTS (
    ...
    AND wa.user_id = workflows.assigned_user_id
);

-- Insert migration record
INSERT INTO migrations (name, applied_at) 
VALUES ('0066_populate_workflow_assignments', NOW())
ON CONFLICT (name) DO NOTHING;

-- =========================================================================
-- Migration 0067: Add Order Delivery Fields
-- =========================================================================
-- These might already exist from 0065, so we check first
ALTER TABLE orders
...
-- Insert migration record
INSERT INTO migrations (name, applied_at) 
VALUES ('0067_add_order_delivery_fields', NOW())
ON CONFLICT (name) DO NOTHING;
...
-- =========================================================================
-- Migration 0070: Fix Project Tags to URLs
-- =========================================================================
-- This migration updates the relationship between project tags and target URLs
-- Specific implementation depends on your schema, adding placeholder
-- You may need to adjust this based on your actual schema

-- Insert migration record
INSERT INTO migrations (name, applied_at) 
VALUES ('0070_fix_project_tags_to_urls', NOW())
ON CONFLICT (name) DO NOTHING;
...
SELECT COUNT(*) as new_migrations_count 
FROM migrations 
WHERE name IN (
    ...
    '0064_add_publisher_coordination_fields',
    '0065_add_order_completion_tracking',
    '0066_populate_workflow_assignments',
    '0067_add_order_delivery_fields',
...
SELECT 
    EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'client_brand_intelligence') as brand_intelligence_exists,
    EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'workflow_assignments') as workflow_assignments_exists;